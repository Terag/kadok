...
func registerHandlers(dg *discordgo.Session, bc *BotContext) error {
	dg.AddHandler(makeMessageCreateHandler(bc))
	return nil
}

// messageCreate is the handler that manages message created events
func makeMessageCreateHandler(bc *BotContext) func(s *discordgo.Session, m *discordgo.MessageCreate) {
	return func(s *discordgo.Session, m *discordgo.MessageCreate) {
		...
		if len(m.Content) >= len(Configuration.Prefix) && strings.ToUpper(m.Content[:len(Configuration.Prefix)]) == strings.ToUpper(Configuration.Prefix) {
			...
			if isGranted(action) {
				response, err := executeAction(bc, s, m)
				if err != nil {
					fmt.Println(err)
					_, err = s.ChannelMessageSend(m.ChannelID, "Oups problème !")
					if err != nil {
