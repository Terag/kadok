...
build:
	go build -ldflags '-X "main.Version=$(BUILD_VERSION)" -X "main.BuildDate=$(BUILD_DATE)" -X "main.GitCommit=$(GIT_COMMIT)" -X "main.Contributors=$(GIT_CONTRIBUTORS)" -X "main.GoVersion=$(GO_VERSION)"'